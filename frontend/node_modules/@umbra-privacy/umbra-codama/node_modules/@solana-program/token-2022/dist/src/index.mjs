...
function getHarvestWithheldTokensToMintInstruction(input, config) {
  ...
  const accounts = originalAccounts;
  const args = { ...input };
  const remainingAccounts = args.sources.map((address) => ({ address, role: AccountRole.WRITABLE }));
  const getAccountMeta = getAccountMetaFactory(programAddress);
  return Object.freeze({
...
function isToken2022Error(error, transactionMessage, code) {
  return isProgramError(error, transactionMessage, TOKEN_2022_PROGRAM_ADDRESS, code);
}
  ...
  return extensions.flatMap((extension2) => {
    switch (extension2.__kind) {
      ...
      case "PermissionedBurn": {
        const authority = isOption(extension2.authority) ? extension2.authority : wrapNullable(extension2.authority);
        if (isNone(authority)) {
